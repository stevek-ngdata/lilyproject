...
/**
 * A record in which the fields are also identified by ID instead of only by QName.
 *
 * <p>In a normal Record, fields are only loaded with their QName. On the storage level, fields are identified
 * by ID. After a Record is read, the QNames of field types can change, making it impossible to reliably map
 * the QName again to the field type (the old QName might now point to another field type). For most applications
 * this will pose no problems. But if you need to know the exact identity of the fields, you can use this class.
 *

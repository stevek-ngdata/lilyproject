...
public class HBaseMetrics {
    ...
    public void reportMetrics(Metrics metrics) throws Exception {
        ...
        for (ServerName serverName : clusterStatus.getServers()) {
            ...
            MBeanServerConnection connection = jmxConnections.getConnector(serverName.getHostname(), HBASE_JMX_PORT).getMBeanServerConnection();
    ...
    public void reportRequestCountMetric(Metrics metrics) throws Exception {
        ClusterStatus clusterStatus = hbaseAdmin.getClusterStatus();
        for (ServerName serverName : clusterStatus.getServers()) {
            metrics.increment("-hbaseRequestCount@" + serverName.getHostname(), clusterStatus.getLoad(serverName).getNumberOfRequests());
        }
    }

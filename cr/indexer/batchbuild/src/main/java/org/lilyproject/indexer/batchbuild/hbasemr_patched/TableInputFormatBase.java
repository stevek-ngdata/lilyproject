...
/**
 * A base for {@link org.apache.hadoop.hbase.mapreduce.TableInputFormat}s. Receives a {@link HTable}, an
...
extends InputFormat<ImmutableBytesWritable, Result> implements Configurable {
  ...
  throws IOException {
    ...
    trr.setScan(sc);
    HTable table = new HTable(new Configuration(getConf()), tableName);
    trr.setHTable(table);
    trr.init();
    return trr;
  }
  ...
  public List<InputSplit> getSplits(JobContext context) throws IOException {
    HTable table = null;
    try {
         table = new HTable(new Configuration(getConf()), tableName);
        ...
        if (table != null) {
            HConnectionManager.deleteConnection(table.getConfiguration(), true);
        }
    }
  }
